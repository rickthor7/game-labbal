...
function resetGame() {
  ...
  if (hardMode) {
    noteElement.innerHTML = `Klik joystick untuk memulai!
        <p>Hard Mode, Hindari lubang hitam. Kembali ke Ezz Mode? Tekan E</p>`;
  } else {
    noteElement.innerHTML = `Klik joystick untuk memulai!
        <p>Pindahkan setiap bola ke tengah. Mau hard mode? Tekan H</p>`;
  }
...
function main(timestamp) {
  ...
  try {
    ...
    if (
      ...
      noteElement.innerHTML = `Selamat! Kamu dapat Sepeda! wkwk
        ${!hardMode ? "<p>Tekan H untuk masuk ke Hard Mode</p>" : ""}
        <p>
          Follow me
          <a href="https://instagram.com/iam.rickthor7" , target="_blank"
            >@iam.rickthor7</a
          >
        </p>`;
      noteElement.style.opacity = 1;
    ...
    if (error.message == "The ball fell into a hole") {
      noteElement.innerHTML = `Masukkan semua bola kedalam lubang. Tekan spasi untuk memulai ulang permainan.
        <p>
          Kembali ke Ezz Mode? Tekan E
        </p>`;
      noteElement.style.opacity = 1;
      gameInProgress = false;
    } else throw error;
